...
int resumeinit(int y, int x, int mines, int **game, char **states, int count) {
	...
	do {	
		...
		fwrite(&count, i, sizeof(int), resf);
		fwrite(&y, 1, sizeof(int), resf);
		fwrite(&x, 1, sizeof(int), resf);
		fwrite(&mines, 1, sizeof(int), resf);
		for(i = 0; i < y; i++) {
			for(j = 0; j < x; j++) {
		...
		fwrite(&flags, 1, sizeof(int), resf);
		fwrite(&flagsonmines, 1, sizeof(int), resf);
		fwrite(&open, 1, sizeof(int), resf);
		fclose(resf);
		if(temp == 1) {
			resf = fopen("resumedata.txt", "w");
			fclose(resf);
